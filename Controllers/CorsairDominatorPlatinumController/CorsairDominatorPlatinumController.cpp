...
void CorsairDominatorPlatinumController::ApplyColors()
{
    ...
    | This is the confirmed minimum recovery from white flash   |
    | (§6 ablation study: "just 2" PASSES, "just 0" FAILS).    |
    | Also ensures the DIMM renders our color data instead of   |
    | a hardware effect.                                        |
    \*---------------------------------------------------------*/
    ...
    )
{
    ...
    /*---------------------------------------------------------*\
    | SwitchMode recovery: single 0x23=0x02 write.              |
    |                                                           |
    | Per §6 ablation study (reset_rainbow.go):                 |
    |   S7 "just 2" PASSES; S5 "just 0" FAILS; S8 none FAILS.  |
    | A single SwitchMode write to direct (0x02) is the         |
    | confirmed minimum recovery from any stuck state. The      |
    | subsequent effect command will switch the DIMM back to    |
    | hardware-controlled mode automatically.                   |
    \*---------------------------------------------------------*/
    SmbusWriteByte(CORSAIR_DOMINATOR_REG_SWITCH_MODE, 0x02);
